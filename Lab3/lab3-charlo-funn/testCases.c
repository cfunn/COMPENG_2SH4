...
//=================Question 2================================
	...
	void TestQ2_manageGrades(CuTest *tc) {

		char inputFile[] =  "inputTest.txt";
		char gradesFile[] =  "gradesTest.txt";
		int size;
		//create list using the input file
		student **Clist = create_class_list(inputFile,&size);
		//input grades using the grades input file
		...
		compute_final_course_grades(Clist,size);
		//Output final course marks
		output_final_course_grades("outputActual.txt",Clist,size);

		//now we open the golden file with the written file and compare line by line

		char actualFile[] =  "outputActual.txt";
		char goldenFile[] =  "outputTest.txt";
		...
		int i;
		for(i=0;i<expectedNumStudents;i++){ // compare each line of actual output file to expected output file
			fscanf(actualPtr,"%d",&actualID);
		...
		// call withdraw() three times, one time when the student is not in the list - check each time whether student is still in the list with find()
		int expected, actual;

		withdraw(1234, Clist, &size);
		expected = -1;
		actual = find(1234, Clist, size);
		CuAssertIntEquals(tc, expected, actual);

		withdraw(1234, Clist, &size); // should print "Student 1234 is not in this class!"

		withdraw(5678, Clist, &size);
		expected = -1;
		actual = find(5678, Clist, size);
		CuAssertIntEquals(tc, expected, actual);

		// make sure the last entry in the list has moved to the left a couple times, making its index 4
		expected = 4;
		actual = find(7891, Clist, size);
		CuAssertIntEquals(tc, expected, actual);
		...
		expected = 0;
		actual = size;
		CuAssertIntEquals(tc, expected, actual); // make sure size is now 0
	}
...
//===========================================================
	CuSuite* Lab3GetSuite() {
		...
		SUITE_ADD_TEST(suite, TestQ2_manageGrades);
